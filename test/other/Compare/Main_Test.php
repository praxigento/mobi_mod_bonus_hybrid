...
class Main_OtherTest extends BaseIntegrationTest {
    ...
    public function __construct() {
        ...
        $this->_toolScheme = $this->_obm->get(\Praxigento\Bonus\Hybrid\Lib\Tool\IScheme::class);
    }
    ...
    private function _addConfigOverride($rankCode, $schemeCode, $gen, $percent) {
        $respGetByCode = $this->_callRank->getByCode(new RankGetByCodeRequest($rankCode));
        $rankId = $respGetByCode->getId();
        $req = new RepoAddEntityRequest(CfgOverride::ENTITY_NAME);
        $req->setBind([
    ...
    private function _createOrders() {
        $reqOrder = new RepoAddEntityRequest(Cfg::ENTITY_MAGE_SALES_ORDER);
        $reqPv = new RepoAddEntityRequest(PvSale::ENTITY_NAME);
        foreach($this->_mapCsvOrders as $orderId => $one) {
            ...
            $respOrder = $this->_callRepo->addEntity($reqOrder);
            $orderId = $respOrder->getIdInserted();
            $reqPv->setBind([
            ...
            $this->_callRepo->addEntity($reqPv);
        }
    }
    ...
    private function _selectCompressionPtc($calcId) {
        ...
        $req = new RepoGetEntitiesRequest(PtcCompression::ENTITY_NAME, $where);
    ...
    private function _selectWalletTransactionsByCalcIds($calcIds) {
        ...
        foreach($calcIds as $calcId) {
            ...
            $respGetEntities = $this->_callRepo->getEntities(new RepoGetEntitiesRequest(LogOpers::ENTITY_NAME, $whereLog));
            $operId = $respGetEntities->getData('/0/' . LogOpers::ATTR_OPER_ID); // get first entry's 'oper_id' attribute
    ...
    private function _setBonusLevelsPersonal() {
        /* DEFAULT */
        $resp = $this->_callTypeCalc->getByCode(new BonusTypeCalcGetByCodeRequest(Cfg::CODE_TYPE_CALC_BONUS_PERSONAL_DEF));
        ...
        /* EU */
        $resp = $this->_callTypeCalc->getByCode(new BonusTypeCalcGetByCodeRequest(Cfg::CODE_TYPE_CALC_BONUS_PERSONAL_EU));
        $calTypeId = $resp->getId();
        $data = [
    ...
    private function _setBonusLevelsTeam() {
        $resp = $this->_callTypeCalc->getByCode(new BonusTypeCalcGetByCodeRequest(Cfg::CODE_TYPE_CALC_BONUS_TEAM_DEF));
        $calTypeId = $resp->getId();
        $data = [
            [ Level::ATTR_CALC_TYPE_ID => $calTypeId, Level::ATTR_LEVEL => 0, Level::ATTR_PERCENT => 0 ],
    ...
    private function _validateCompressionPtc() {
        ...
        foreach($data as $one) {
            ...
            if(!is_null($targetItem)) {
                ...
                if(
                    ...
                    $wrongItems[$custMlmId] = [ 'expect' => [ $targetParentMlmId, $targetPv ], 'act' => [ $parentMlmId, $pv ] ];
                    $this->_logger->debug("Wrong PTC compress validation: customer #$custMlmId (#$custId) " . var_export($wrongItems[$custMlmId], true));
                } else {
                    /* correct actual item */
        ...
        $this->assertTrue(count($missed) == 0, "There are some missed items in the compressed downline tree (PTC)." . var_export($missed, true));
    }
        ...
        foreach($data as $one) {
            ...
            if(!is_null($targetItem)) {
                ...
                if((abs($tv - $expectTv) > Cfg::DEF_ZERO)) {
                    ...
                    $this->_logger->debug("Wrong TV value: customer #$custMlmId (#$custId ) " . var_export($wrongItems[$custMlmId], true));
                }
            } else {
